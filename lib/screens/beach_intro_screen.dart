...
import 'beach_sort_transition_screen.dart';
...
/// Beach intro: zoom into polluted shoreline, Marine Ranger then Mayor.
class BeachIntroScreen extends StatefulWidget {
  const BeachIntroScreen({
    super.key,
  ...
  @override
  State<BeachIntroScreen> createState() => _BeachIntroScreenState();
}

class _BeachIntroScreenState extends State<BeachIntroScreen>
  ...
  static const List<_Line> _lines = <_Line>[
    _Line(
      'Marine Ranger',
      _rangerAccent,
      'Welcome to Green Beach.',
    ),
    _Line(
      'Marine Ranger',
      _rangerAccent,
      'Our ocean provides food, beauty, and life, but pollution is threatening everything that lives here.',
    ),
    _Line(
      'Mayor',
      _mayorAccent,
      'Every piece of trash left behind can eventually reach the sea.',
      isMayor: true,
    ),
    _Line(
      'Marine Ranger',
      _rangerAccent,
      "Let's help clean the beach and protect our marine life.",
    ),
  ];
  ...
  late final AnimationController _rangerIn = AnimationController(
    vsync: this,
  ...
  void initState() {
    ...
    _rangerIn.addStatusListener((AnimationStatus status) {
      if (status == AnimationStatus.completed) {
        _advanceLine();
      }
    });
    ...
    Navigator.of(context).pushReplacement(
      MaterialPageRoute<void>(
        builder: (BuildContext context) => PuzzleScreen(
          ...
          imageAsset: GameProgress.beachPuzzleClean,
          itemName: 'Clean Beach',
          gridSize: 4,
          backgroundAsset: GameProgress.beachTrashBg,
          phaseLabel: 'Level 7 · Phase 1',
          instructionText:
              'Complete the picture to see what our beach can become.',
          completeMessage: "Now let's remove the waste from the beach.",
          nextBuilder: (BuildContext context) => BeachSortTransitionScreen(
            character: widget.character,
            location: widget.location,
          ),
        ),
              ...
              children: <Widget>[
                  ...
                  child: ClipRect(
                    child: AnimatedBuilder(
                      ...
                      builder: (BuildContext context, Widget? child) {
                        final double scale = 1.0 + 0.24 * _zoom.value;
                ...
                if (!compact) ...<Widget>[
                  AnimatedBuilder(
                    ...
                    builder: (BuildContext context, Widget? child) {
                      final double t =
                          Curves.easeOut.transform(_rangerIn.value);
                      return Positioned(
                        bottom: 0 - (1 - t) * 36,
                        left: w * 0.04,
                        height: spriteHeight,
