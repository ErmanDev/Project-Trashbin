import 'dart:async';

import 'package:flutter/material.dart';

import '../models/game_character.dart';
import '../models/town_location.dart';
import '../widgets/dialogue_box.dart';
import '../widgets/pixel_button.dart';
import 'puzzle_screen.dart';

/// A single spoken line, kept with its speaker + accent so the dialogue
/// system can already switch speakers later.
class _Line {
  const _Line(this.speaker, this.accent, this.text);
  final String speaker;
  final Color accent;
  final String text;
}

/// Level 1 story cinematic for the Park.
///
/// The camera zooms into the trash-covered, empty park while fading in from
/// black, then the Mayor appears and delivers his line. When he finishes, a
/// button appears to start the actual level.
class ParkIntroScreen extends StatefulWidget {
  const ParkIntroScreen({
    super.key,
    required this.character,
    required this.location,
  });

  final GameCharacter character;
  final TownLocation location;

  @override
  State<ParkIntroScreen> createState() => _ParkIntroScreenState();
}

class _ParkIntroScreenState extends State<ParkIntroScreen>
    with TickerProviderStateMixin {
  static const Color _mayorAccent = Color(0xFF3949AB);

  static const List<_Line> _lines = <_Line>[
    _Line('Mayor', _mayorAccent, "Let's start by cleaning our park."),
  ];

  static const Duration _pauseBetweenLines = Duration(seconds: 1);
  static const Duration _charTick = Duration(milliseconds: 30);

  // Camera zoom + fade-in of the scene.
  late final AnimationController _zoom = AnimationController(
    vsync: this,
    duration: const Duration(milliseconds: 2600),
  );
  // Mayor fading/rising into view once the zoom settles.
  late final AnimationController _mayorIn = AnimationController(
    vsync: this,
    duration: const Duration(milliseconds: 700),
  );

  int _lineIndex = -1;
  String _shownText = '';
  bool _typing = false;
  bool _showReady = false;

  Timer? _typeTimer;
  Timer? _pauseTimer;

  @override
  void initState() {
    super.initState();

    _zoom.addStatusListener((AnimationStatus status) {
      if (status == AnimationStatus.completed) {
        _mayorIn.forward();
      }
    });
    _mayorIn.addStatusListener((AnimationStatus status) {
      if (status == AnimationStatus.completed) {
        _advanceLine();
      }
    });

    _zoom.forward();
  }

  @override
  void dispose() {
    _typeTimer?.cancel();
    _pauseTimer?.cancel();
    _zoom.dispose();
    _mayorIn.dispose();
    super.dispose();
  }

  void _advanceLine() {
    if (!mounted) return;
    final int next = _lineIndex + 1;
    if (next >= _lines.length) {
      setState(() => _showReady = true);
      return;
    }
    setState(() => _lineIndex = next);
    _typeLine(_lines[next].text);
  }

  void _typeLine(String full) {
    _typeTimer?.cancel();
    setState(() {
      _shownText = '';
      _typing = true;
    });
    int shown = 0;
    _typeTimer = Timer.periodic(_charTick, (Timer timer) {
      if (shown >= full.length) {
        timer.cancel();
        setState(() => _typing = false);
        _pauseTimer = Timer(_pauseBetweenLines, _advanceLine);
        return;
      }
      shown++;
      setState(() => _shownText = full.substring(0, shown));
    });
  }

  /// Tap to speed up: finish the current line instantly, or skip the pause.
  void _onTap() {
    if (_showReady || _lineIndex < 0) return;
    if (_typing) {
      _typeTimer?.cancel();
      _pauseTimer?.cancel();
      setState(() {
        _shownText = _lines[_lineIndex].text;
        _typing = false;
      });
      _pauseTimer = Timer(_pauseBetweenLines, _advanceLine);
    } else {
      _pauseTimer?.cancel();
      _advanceLine();
    }
  }

  void _onStart() {
    Navigator.of(context).pushReplacement(
      MaterialPageRoute<void>(
        builder: (BuildContext context) => PuzzleScreen(
          location: widget.location,
          character: widget.character,
          imageAsset: 'assets/images/png/puzzle_recycling_bin.png',
          itemName: 'Recycling Bin',
        ),
      ),
    );
  }

  @override
  Widget build(BuildContext context) {
    return Scaffold(
      backgroundColor: Colors.black,
      body: GestureDetector(
        onTap: _onTap,
        child: LayoutBuilder(
          builder: (BuildContext context, BoxConstraints constraints) {
            final double w = constraints.maxWidth;
            final double h = constraints.maxHeight;
            final bool compact = h < 420;
            final double spriteHeight = h * 0.72;
            final double bgOpacityCap = compact ? 0.42 : 1.0;

            return Stack(
              clipBehavior: Clip.none,
              children: <Widget>[
                // Trashed park background, zooming in as the scene fades in.
                Positioned.fill(
                  child: ClipRect(
                    child: AnimatedBuilder(
                      animation: _zoom,
                      builder: (BuildContext context, Widget? child) {
                        final double scale = 1.0 + 0.22 * _zoom.value;
                        final double opacity = compact
                            ? (0.08 + _zoom.value * bgOpacityCap)
                                .clamp(0.0, bgOpacityCap)
                            : (0.15 + _zoom.value).clamp(0.0, 1.0);
                        return Transform.scale(
                          scale: scale,
                          child: Opacity(
                            opacity: opacity,
                            child: child,
                          ),
                        );
                      },
                      child: Image.asset(
                        'assets/images/png/park_trash_bg.png',
                        fit: BoxFit.cover,
                        filterQuality: FilterQuality.none,
                      ),
                    ),
                  ),
                ),

                // Soft vignette to focus the eye.
                Positioned.fill(
                  child: DecoratedBox(
                    decoration: BoxDecoration(
                      gradient: RadialGradient(
                        radius: 1.1,
                        colors: <Color>[
                          Colors.transparent,
                          Color(compact ? 0x77000000 : 0x55000000),
                        ],
                        stops: const <double>[0.6, 1.0],
                      ),
                    ),
                  ),
                ),

                // Extra bottom dim on mobile so the mayor + dialogue read clearly.
                if (compact)
                  const Positioned.fill(
                    child: DecoratedBox(
                      decoration: BoxDecoration(
                        gradient: LinearGradient(
                          begin: Alignment.topCenter,
                          end: Alignment.bottomCenter,
                          colors: <Color>[
                            Colors.transparent,
                            Color(0x66000000),
                            Color(0xAA000000),
                          ],
                          stops: <double>[0.35, 0.72, 1.0],
                        ),
                      ),
                    ),
                  ),

                // Initial fade-from-black.
                Positioned.fill(
                  child: IgnorePointer(
                    child: AnimatedBuilder(
                      animation: _zoom,
                      builder: (BuildContext context, _) => ColoredBox(
                        color: Colors.black
                            .withValues(alpha: (1 - _zoom.value).clamp(0.0, 1.0)),
                      ),
                    ),
                  ),
                ),

                // Mayor bust on desktop (mobile bust lives in the dialogue stack).
                if (!compact)
                  AnimatedBuilder(
                    animation: _mayorIn,
                    builder: (BuildContext context, Widget? child) {
                      final double t =
                          Curves.easeOut.transform(_mayorIn.value);
                      return Positioned(
                        bottom: 0 - (1 - t) * 40.0,
                        left: w * 0.05,
                        height: spriteHeight,
                        child: Opacity(opacity: t, child: child),
                      );
                    },
                    child: Image.asset(
                      'assets/images/png/char_mayor_cutout.png',
                      filterQuality: FilterQuality.none,
                    ),
                  ),

                // Dialogue + start button (layered on mobile so the mayor sits
                // behind the speaker name tag, Ren'Py style).
                if (_lineIndex >= 0)
                  Positioned(
                    left: compact ? 10 : 16,
                    right: compact ? 10 : 16,
                    bottom: compact ? 10 : 16,
                    child: SafeArea(
                      top: false,
                      child: compact
                          ? _buildMobileDialogueStack(
                              w: w,
                              h: h,
                              line: _lines[_lineIndex],
                            )
                          : Column(
                              mainAxisSize: MainAxisSize.min,
                              children: <Widget>[
                                if (_showReady) ...<Widget>[
                                  PixelButton(
                                    label: 'Start Cleaning!',
                                    icon: Icons.cleaning_services,
                                    color: const Color(0xFF4CAF50),
                                    width: null,
                                    onPressed: _onStart,
                                  ),
                                  const SizedBox(height: 14),
                                ],
                                DialogueBox(
                                  text: _shownText,
                                  speakerName: _lines[_lineIndex].speaker,
                                  accent: _lines[_lineIndex].accent,
                                  showContinueHint:
                                      !_typing && !_showReady,
                                ),
                              ],
                            ),
                    ),
                  ),
              ],
            );
          },
        ),
      ),
    );
  }

  /// Ren'Py layering on mobile: mayor → text panel → speaker tag on top.
  Widget _buildMobileDialogueStack({
    required double w,
    required double h,
    required _Line line,
  }) {
    const double textPanelMinH = 96;
    const double tagH = 32;

    return Stack(
      clipBehavior: Clip.none,
      alignment: Alignment.bottomLeft,
      children: <Widget>[
        // 1) Mayor bust (back layer).
        AnimatedBuilder(
          animation: _mayorIn,
          builder: (BuildContext context, Widget? child) {
            final double t = Curves.easeOut.transform(_mayorIn.value);
            return Positioned(
              left: 0,
              bottom: textPanelMinH - 8,
              width: w * 0.40,
              child: Opacity(
                opacity: t,
                child: ClipRect(
                  child: Align(
                    alignment: Alignment.topCenter,
                    heightFactor: 0.50,
                    child: Transform.flip(flipX: true, child: child),
                  ),
                ),
              ),
            );
          },
          child: Image.asset(
            'assets/images/png/char_mayor_cutout.png',
            width: w * 0.40,
            fit: BoxFit.fitWidth,
            alignment: Alignment.topCenter,
            filterQuality: FilterQuality.none,
          ),
        ),

        // 2) Button + text panel (middle layer).
        Column(
          mainAxisSize: MainAxisSize.min,
          crossAxisAlignment: CrossAxisAlignment.stretch,
          children: <Widget>[
            if (_showReady) ...<Widget>[
              Center(
                child: PixelButton(
                  label: 'Start Cleaning!',
                  icon: Icons.cleaning_services,
                  color: const Color(0xFF4CAF50),
                  width: null,
                  compact: true,
                  onPressed: _onStart,
                ),
              ),
              const SizedBox(height: 14),
            ],
            Padding(
              padding: const EdgeInsets.only(top: tagH - 4),
              child: DialogueBox(
                text: _shownText,
                accent: line.accent,
                showContinueHint: !_typing && !_showReady,
                showSpeakerName: false,
              ),
            ),
          ],
        ),

        // 3) Speaker name tag (front layer, on top of mayor).
        Positioned(
          left: 0,
          bottom: textPanelMinH,
          child: SpeakerNameTag(
            name: line.speaker,
            accent: line.accent,
            compact: true,
          ),
        ),
      ],
    );
  }
}
